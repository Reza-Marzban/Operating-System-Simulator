...
public class Cpu_Stack{
	...
	public int[] pop()
	{
		...
		if(TOS>0){
			...
			errorhandler.error_handler_routine("StackEmpty",CPU.active_pcb);//Error_handler (Stack Empty)
			return i;
		}
		return i;
	}
	public void push(int[] a){
		if(TOS<7){
			...
			if(length==16)
			{
				...
				errorhandler.error_handler_routine("BadInput",CPU.active_pcb);//Error_handler (Bad Input)
			}
		}else{
			errorhandler.error_handler_routine("StackFull",CPU.active_pcb);//Error_handler (Stack Full)
		}
	}
}